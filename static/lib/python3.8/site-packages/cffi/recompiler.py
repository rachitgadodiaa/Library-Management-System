...
class Recompiler:
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        if self.ffi._embedding is not None:
            ...
            prnt('static_in_env const char _CFFI_PYTHON_STARTUP_CODE[] = {')
        ...
        prnt('static_in_env void *_cffi_types[] = {')
        ...
        for step_name in self.ALL_STEPS:
            ...
            if nums[step_name] > 0:
                prnt('static_in_env const struct _cffi_%s_s _cffi_%ss[] = {' % (
                    step_name, step_name))
                for entry in lst:
        ...
        if self.ffi._included_ffis:
            prnt('static_in_env const char * const _cffi_includes[] = {')
        ...
        prnt('static_in_env const struct _cffi_type_context_s _cffi_type_context = {')
        prnt('  _cffi_types,')
        for step_name in self.ALL_STEPS:
            if nums[step_name] > 0:
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        prnt('static_in_env %s' % (tp.result.get_c_name(name_and_arguments),))
        prnt('{')
        ...
        if difference:
            ...
            name_and_arguments = '%s_cffi_f_%s(%s)' % (abi, name,
                                                       repr_arguments)
            prnt('static_in_env %s' % (tp_result.get_c_name(name_and_arguments),))
            prnt('{')
            if result_decl:
                prnt(result_decl)
            call_arguments = ', '.join(call_arguments)
    ...
    def _struct_decl(self, tp, cname, approxname):
        ...
        prnt('static_in_env void %s(%s *p)' % (checkfuncname, cname))
        prnt('{')
    ...
    # ----------
    # constants, declared with "static_in_env const ..."
    ...
    def _generate_cpy_const(self, is_int, name, tp=None, category='const',
        ...
        if is_int:
            prnt('static_in_env int %s(unsigned long long *o)' % funcname)
            prnt('{')
            prnt('  int n = (%s) <= 0;' % (name,))
            ...
            prnt('static_in_env void %s(char *o)' % funcname)
            prnt('{')
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        ...
        prnt('static_in_env ' + tp.get_c_name(decl, quals=self._current_quals))
        prnt('{')
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        prnt('static_in_env struct _cffi_externpy_s _cffi_externpy__%s =' % name)
        prnt('  { "%s.%s", %s, 0, 0 };' % (
    ...
    def _generate_cpy_extern_python_decl(self, tp, name):
        self._extern_python_decl(tp, name, 'static_in_env ')
